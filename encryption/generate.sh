...
if [[ -f $KEYFILE ]]; then
    ...
    (echo -n '1;' ; sudo openssl rand -hex 32 ) | sudo tee "$KEYFILE" > /dev/null
fi
...
if [[ -f $KEYFILE_KEY ]]; then
    ...
    sudo sudo openssl rand -hex 128 | sudo tee "$KEYFILE_KEY" > /dev/null
fi
...
if [[ -f $KEYFILE_ENC ]]; then
    ...
    sudo sudo openssl enc -aes-256-cbc -md sha1 -pass file:"$KEYFILE_KEY" -in "$KEYFILE" -out "$KEYFILE_ENC"
fi
