...
openssl req -new -x509 -nodes -days 0 -hours 4 -key ca-key.pem -out ca-cert.pem -subj "/CN=mariadb_ssl_CA"

# Generate the server key and certificate
openssl req -newkey rsa:2048 -days 0 -hours 4 -nodes -keyout server-key.pem -out server-req.pem -subj "/CN=mariadb_ssl_server"
...
openssl req -newkey rsa:2048 -days 0 -hours 4 -nodes -keyout client-key.pem -out client-req.pem -subj "/CN=mariadb_ssl_client"
...
openssl req -newkey rsa:2048 -days 0 -hours 4 -nodes -keyout maxscale-key.pem -out maxscale-req.pem -subj "/CN=mariadb_ssl_maxscale"
