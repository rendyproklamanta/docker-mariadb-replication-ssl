...
openssl x509 -req -in master-server-req.pem -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -out master-server-cert.pem -days 365 -sha256

# Generate client key and certificate for the MariaDB master
openssl genrsa -out master-client-key.pem 2048
openssl req -new -key master-client-key.pem -out master-client-req.pem -subj "/CN=$MASTER_CN-client"
openssl x509 -req -in master-client-req.pem -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -out master-client-cert.pem -days 365 -sha256
...
openssl x509 -req -in slave1-server-req.pem -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -out slave1-server-cert.pem -days 365 -sha256

# Generate client key and certificate for the MariaDB slave
openssl genrsa -out slave1-client-key.pem 2048
openssl req -new -key slave1-client-key.pem -out slave1-client-req.pem -subj "/CN=$SLAVE1_CN-client"
openssl x509 -req -in slave1-client-req.pem -CA ca-cert.pem -CAkey ca-key.pem -CAcreateserial -out slave1-client-cert.pem -days 365 -sha256
